...
def parse_html(soup,query,location):
    ...
    for i in soup:
            ...
            link="https://www.pracuj.pl"+ \
                i.find(class_='o-list_item_link_name').get('href')
            ...
            get_data(link,id,company,sponsored,job_title,location,query)
            
            
def get_data(link,id,company,sponsored,job_title,location,query):
    try:
    ...
    if offer_description:
        offer_description = offer_description.get_text(separator=' ')
    else:
        offer_description = soup.find('div', \
            attrs={'class':'o-main__left_table uploaded'}).get_text(separator=' ')
    try:
        language = detect(offer_description)
    except:
        language = '0'
    if language=='en':
        adress = soup.find('span', \
            attrs={'class':'o-main__right_offer_cnt_details_item_text ico-location latlng'})
        lat = adress.get('data-lat')
        long = adress.get('data-lng')
        employ_type = soup.find('span', \
                    attrs={'itemprop':'employmentType'}).text.replace('\n',' ')
        ...
        validThrough = soup.find('span', \
                    attrs={'itemprop':'validThrough'}).string.replace(')','')
        if '.' in validThrough:
            validThrough = datetime.datetime.strptime(validThrough, "%d.%m.%Y" )
        else:
            validThrough = datetime.datetime.strptime(validThrough, "%Y-%m-%d" )
        save_to_database(id,link,location,lat,long,query,company,job_title,\
            employ_type,date_posted,validThrough,sponsored,language,keywords,\
            offer_description)

    
def save_to_database(id,link,location,lat,long,query,company,job_title,\
                     employ_type,date_posted,validThrough,sponsored,language,\
                     keywords,offer_description):
    ...
    cursor = conn.cursor()
    insert_data_sql = '''INSERT into NEW_OFFERS \
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '''
    try:
        cursor.execute(insert_data_sql,(id,link,location,lat,long,query,company,\
                                        job_title,employ_type,date_posted,validThrough,\
                                        sponsored,language,keywords,offer_description))    
    except:
        pass
